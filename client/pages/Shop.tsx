...
const ROAST_IMAGES: Record<RoastLevel, string> = {
  Light:
    "https://cdn.builder.io/api/v1/image/assets%2F7295d6a03e5244e6951bcbaefaa83fce%2F1d8113e0c1664f36aaf13f7866b218c8?format=webp&width=800",
  Medium:
    "https://cdn.builder.io/api/v1/image/assets%2F7295d6a03e5244e6951bcbaefaa83fce%2F8aa582b382db43f09baabde71490667a?format=webp&width=800",
  "Medium Dark":
    "https://cdn.builder.io/api/v1/image/assets%2F7295d6a03e5244e6951bcbaefaa83fce%2F044fb3dd7c434883ac98b1e6865b42ea?format=webp&width=800",
};
...
export default function Shop() {
  ...
  const price = useMemo(
    () => (size ? SIZE_PRICES[size].current * qty : 0),
    [size, qty],
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (tally) {
      ...
      alert(
        "Order summary copied. Add ?tally=YOUR_TALLY_FORM_URL to prefill a Tally form.",
      );
    }
  };
  ...
  return (
    <main className="min-h-screen bg-background">
          ...
          <nav className="mb-8 md:mb-10 flex flex-wrap gap-3 text-sm">
            {[
              ...
              <a
                key={l.id}
                href={`#${l.id}`}
                className="rounded-full bg-white/20 ring-1 ring-white/30 px-3 py-1.5 text-cream hover:bg-white/30 transition-colors"
              >
                {l.label}
              </a>
            ))}
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            ...
            <div className="order-2 lg:order-1">
              ...
              <p className="mt-4 text-sm text-muted-foreground">
                Image updates with roast selection. Prices are placeholders; you
                can update later.
              </p>
            </div>
            ...
            <div className="order-1 lg:order-2">
              <h1 className="font-display text-3xl md:text-4xl text-cream inline-block relative after:absolute after:left-0 after:-bottom-2 after:h-1 after:w-24 after:bg-gold">
                Single Origin Coffee
              </h1>
              <p className="mt-4 text-cream/90 max-w-prose">
                Configure your coffee: choose type, roast level, size, and
                quantity. Enjoy freshly roasted Ethiopian Arabica.
              </p>
              ...
              <div id="roast" className="mt-8 scroll-mt-28">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {(["Light", "Medium", "Medium Dark"] as RoastLevel[]).map(
                    (r) => (
                      <button
                        key={r}
                        type="button"
                ...
                <input
                  ...
                  step={1}
                  value={(
                    ["Light", "Medium", "Medium Dark"] as RoastLevel[]
                  ).indexOf(roast)}
                  onChange={(e) =>
                    setRoast(
                      (["Light", "Medium", "Medium Dark"] as RoastLevel[])[
              ...
              <div id="size" className="mt-8 scroll-mt-28">
                ...
                <div className="mt-3 grid grid-cols-3 gap-3">
                  {(Object.keys(SIZE_PRICES) as Array<unknown> as Size[]).map(
                    (s) => (
                      <button
                        key={s}
                        type="button"
                        ...
                        className={cn(
                          ...
                          size === s
                            ? "ring-gold/60 shadow bg-gold/10"
                            : "ring-border hover:bg-white",
                        )}
                        aria-pressed={size === s}
                      >
                        <div className="text-xs text-foreground/70">{s} g</div>
                        <div className="text-lg font-semibold text-gold">
                          ETB {SIZE_PRICES[s].current.toLocaleString()}
                        </div>
                        {SIZE_PRICES[s].old ? (
                          <div className="text-xs line-through text-foreground/50">
                            ETB {SIZE_PRICES[s].old!.toLocaleString()}
                          </div>
                        ) : null}
                      </button>
                    ),
              ...
              <div className="mt-10 flex items-center justify-between">
                ...
                <a
                  ...
                  className={cn(
                    ...
                    canAdd
                      ? "bg-gold text-coffee hover:shadow-md active:scale-95"
                      : "bg-muted text-foreground/50 cursor-not-allowed pointer-events-none",
                  )}
      ...
      <section id="order" className="relative py-16">
        <div className="container">
          <h2 className="font-display text-2xl md:text-3xl text-foreground inline-block relative after:absolute after:left-0 after:-bottom-2 after:h-1 after:w-20 after:bg-gold">
            Order
          </h2>
          <p className="mt-3 text-foreground/80">
            Fill your details below. Append ?tally=YOUR_TALLY_FORM_URL to this
            page URL to submit into Tally with selections prefilled.
          </p>
          <form onSubmit={handleSubmit} className="mt-8 grid gap-6 max-w-xl">
            ...
            <div className="flex items-center justify-between">
              <div className="hidden sm:block">
                ...
                <pre className="text-foreground/90 text-sm whitespace-pre-wrap max-w-md">
                  {composeSummary()}
                </pre>
              </div>
              <button
                type="submit"
                className="rounded-full bg-gold text-coffee px-6 py-3 font-semibold shadow-sm hover:shadow-md active:scale-95"
              >
                Submit
              </button>
            </div>
          ...
          <div className="rounded-2xl bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/65 ring-1 ring-border shadow-lg p-4 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              <img
                src={ROAST_IMAGES[roast]}
                alt={`${roast} pack`}
                className="h-12 w-10 object-contain rounded-md ring-1 ring-border/60"
              />
              <div className="min-w-0">
                <div className="font-semibold text-foreground truncate">
                  {type} · {roast} · {size ? `${size}g` : "Select size"}
                </div>
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:inline-flex items-center rounded-full ring-1 ring-border overflow-hidden bg-white/60">
                <button
                  type="button"
                  onClick={() => setQty((q) => Math.max(1, q - 1))}
                  className="px-3 py-2 text-lg hover:bg-white"
                  aria-label="Decrease quantity"
                >
                  −
                </button>
